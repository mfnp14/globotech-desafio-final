  ...
  const mockDataRestaurant = [
    {
      ...
      description:
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed consectetur aliquet odio nec gravida. Fusce vitae ornare massa.",
      urlImage:
        ...
        <section className={"welcomeTitle"}>
          ...
          <Link to={"/home"}>
            <section className={"buttonPrimary"}>
              <p className={"textButtonPrimary"}>Começar</p>
            </section>
          </Link>
        </section>
        ...
        <section className={"contentRestaurantList"}>
          ...
          {mockDataRestaurant.map((restaurant) => (
            <div className={"contentCard"}>
              <img className={"imageCard"} src={restaurant.urlImage} alt="photo-food" />
              <h3 className={"titleCard"}>{restaurant.name}</h3>
              <p className={"descriptionCard"}>{restaurant.description}</p>
              <button className={"buttonCard"}>
                <p className={"textButtonCard"}>Ver prato completo</p>
              </button>
            </div>
          ))}
